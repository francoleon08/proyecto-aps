...
import type { QuoteData } from "@/components/user-cotizer-policy";
...
export function QuoteSummaryStep({
  ...
  const finalPrice = quoteData.selectedPlan
    ? Math.round(
        quoteData.basePrices[quoteData.selectedPlan] * quoteData.multiplier
      )
    : 0;
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Resumen de tu póliza de seguro
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="p-6 bg-accent/10 rounded-lg border border-accent/20">
          ...
          <div className="flex items-center gap-2 text-foreground font-mono text-sm flex-wrap">
            <span className="bg-primary text-primary-foreground px-3 py-1 rounded font-bold">
              ${finalPrice}
            </span>
          </div>
        </div>
